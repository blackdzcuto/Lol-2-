...
var full = [];
                ...
                switch (args[0]) {
                    ...
                    case "tôm":
                        case "Tôm": itemm = "tom";
                                icon = '🦞';
                            break;
                                default: return api.sendMessage("[𝙈𝙄𝘿𝘼𝙎] => Hãy Bấm : /baucua [bầu/cua/cá/nai/gà/tôm] [số tiền]",event.threadID, event.messageID);
                }      
                await get(number[0],number[1],number[2]);
            api.sendMessage({body:"[𝙈𝙄𝘿𝘼𝙎] => Đang Đập, À Không Đang Lắc!",attachment: createReadStream(__dirname + "/cache/baucua.gif")},event.threadID,async (error,info) => {
    ...
    var array = [number[0],number[1],number[2]];
        ...
        if (array.includes(itemm)) {
            ...
            else return api.sendMessage("[𝙈𝙄𝘿𝘼𝙎 => Lỗi ! Code : XX1N",event.threadID,event.messageID);
        } else  {
